...
const Game = dynamic(() => import('@/lib/components/game/Game'), {
    ssr: false,
    loading: () => <Spinner size="lg" color="secondary" aria-label="Map loading..." />
});
...
const Main = () => {
    ...
    useEffect(() => {
        ...
        const arrayfied =
            category === 'all' ? 'all' : Array.from(new Set(category?.split(',') ?? []));
    ...
    return (
        <>
            <div className="grow flex justify-center items-center p-2 sm:p-4 lg:p-6 xl:p-8 overflow-hidden size-full">
                <Game gameState={gameState} info={infoState} setInfo={setInfoState} />
            </div>
            <GameBar gameState={gameState} info={infoState} setInfo={setInfoState} />
        </>
    );
};
