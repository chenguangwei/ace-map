...
import { type RefObject, useCallback, useMemo, useReducer, useRef } from 'react';
import { getPlace, isNearlyCorrect, type Place, type PlaceWithoutName, Strictness } from './places';
...
type Action =
    ...
    | { type: 'SET_CURRENT_MARKER'; payload: PlaceWithoutName | 'submitted' | 'none' }
...
export const useGame = (): GameState => {
    ...
    const availablePlaces = useMemo(() => getPlace(state.category), [state.category]);
    ...
    const setCurrentMarker = useCallback((marker: PlaceWithoutName | 'submitted' | 'none') => {
        dispatch({ type: 'SET_CURRENT_MARKER', payload: marker });
    }, []);

    const submitMarker = useCallback((): SubmitInfo => {
        if (state.currentMarker === 'none' || state.currentMarker === 'submitted' || !toMark) {
            return null;
        }

        const isCorrect = isNearlyCorrect(state.currentMarker, toMark, state.strictness);
